    ...
    <body>
                        ...
                        <div class="row">
                            <div class="col-xs-12">
                                <div class="page-title-box">
                                    <h4 class="page-title">Room Area</h4>
                        ...
                        <div class="row">
                            ...
                            <div id="msg"></div>
                                <div class="card-box table-responsive">
                                    <h4 class="m-t-0 header-title"><b>Room Area Tables</b></h4>
                                    <p class="text-muted font-14 m-b-30">
                                    </p>
                                    <div class="row">
                                        <div class="col-sm-6">
                                            <div class="m-b-30">
                                                <a href="<?php echo base_url(); ?>adm/Room/add_room_area" id="addToTable" class="btn btn-success waves-effect waves-light">Add <i class="mdi mdi-plus-circle-outline"></i></a>
                                            </div>
                                        </div>
                                    </div>
        ...
        <div id="Delete" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="javascript:delete_room_area();" method="post" id="form-delete">
                    <div class="modal-body">
                        <input type="hidden" name="room_area" id="room_area" value=""/>
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                view_table_room_area();
                $('#datatable').dataTable();
            });
          
            function view_table_room_area(){
                $.ajax({
                    type  : 'ajax',
                    url   : '<?php echo base_url(); ?>adm/Room/view_room_area',
                    async : false,
                    dataType : 'json',
                    success : function(data){
                        ...
                        for(i=0; i<data.length; i++){
                            var datas = '\''+data[i].id_room_area+'\'';
                            html += '<tr>'+
                                    '<td>'+ no++ +'</td>'+
                                    '<td>'+data[i].room_area_name+'</td>'+
                                    '<td>'+data[i].room_area_createdate+'</td>'+
                                    '<td class="actions"><a href="<?php echo base_url(); ?>adm/Room/view_update_room_area/'+data[i].id_room_area+'"><button type="button" class="btn btn-icon waves-effect waves-light btn-primary"> <i class="fa fa-pencil"></i> </button></a> <button type="button" data-toggle="modal" data-target="#Delete" onclick="javascript:setValueidroomarea('+datas+');" class="btn btn-icon waves-effect waves-light btn-danger"> <i class="fa fa-trash"></i> </button></td>'+
                                    '</tr>';
                        }
                        $('#show_data_room_area').html(html);
                    }
     
                });
            }
            function setValueidroomarea(id_room_area)
            {
                $("#room_area").val(id_room_area);
            }

            function delete_room_area()
            {
                $.ajax({
                    url:"<?php echo base_url() ?>adm/Room/delete_room_area",
                    type:"POST",
                    dataType:"JSON",
