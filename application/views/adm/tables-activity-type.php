    ...
    <body>
                        ...
                        <div class="row">
                            <div class="col-xs-12">
                                <div class="page-title-box">
                                    <h4 class="page-title">Activity Type</h4>
                                    <ol class="breadcrumb p-0 m-0">
                                        <li>
                                            <a href="<?php echo base_url(); ?>adm/">Dashboard</a>
                                        </li>
                                        <li class="active">
                                            Activity Type
                                        </li>
                        ...
                        <div class="row">
                            ...
                            <div id="msg"></div>
                                <div class="card-box table-responsive">
                                    <h4 class="m-t-0 header-title"><b>Activity Type Tables</b></h4>
                                    <p class="text-muted font-14 m-b-30">
                                    </p>
                                    <div class="row">
                                        <div class="col-sm-6">
                                            <div class="m-b-30">
                                                <a href="<?php echo base_url(); ?>adm/Activity_type/add_activity_type" id="addToTable" class="btn btn-success waves-effect waves-light">Add <i class="mdi mdi-plus-circle-outline"></i></a>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <table id="datatable" class="table table-striped table-bordered">
                                        ...
                                        <tr>
                                            <th>#</th>
                                            <th>Activity Name</th>
                                            <th>Activity Create Date</th>
                                            <th>Action</th>
                                        </tr>
                                        </thead>


                                        <tbody id="show_data_activity_data">
                                        </tbody>
                                    </table>
                                </div>
                            </div>
        ...
        <div id="Delete" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="javascript:delete_activity_type();" method="post" id="form-delete">
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                view_table_activity_type();
                $('#datatable').dataTable();
            });
          
            function view_table_activity_type() {
                $.ajax({
                    type: 'ajax',
                    url: '<?php echo base_url(); ?>adm/Activity_type/view_table_activity_type',
                    async: false,
                    dataType: 'json',
                    success: function (data) {
                        ...
                        for (i = 0; i < data.length; i++) {
                            var datas = '\'' + data[i].id_activity + '\'';
                            html += '<tr>' +
                                '<td>' + no++ + '</td>' +
                                '<td>' + data[i].activity_name + '</td>' +
                                '<td>' + data[i].activity_createdate + '</td>' +
                                '<td class="actions"><a href="<?php echo base_url(); ?>adm/Activity_type/view_update_activity/' + data[i].id_activity + '"><button type="button" class="btn btn-icon waves-effect waves-light btn-primary"> <i class="fa fa-pencil"></i> </button></a> <button type="button" data-toggle="modal" data-target="#Delete" onclick="javascript:setValueActivityType(' + datas + ');"class="btn btn-icon waves-effect waves-light btn-danger"> <i class="fa fa-trash"></i> </button></td>' +
                                '</tr>';
                        }
                        $('#show_data_activity_data').html(html);
                    }

                });
            }

            function setValueActivityType(id_activity)
            {
                $("#activity_type").val(id_activity);
            }

            function delete_activity_type()
            {
                $.ajax({
                    url:"<?php echo base_url() ?>adm/Activity_type/delete_activity_type",
                    type:"POST",
                    dataType:"JSON",
