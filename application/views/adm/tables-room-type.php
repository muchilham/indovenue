    ...
    <body>
                        ...
                        <div class="row">
                            ...
                            <div id="msg"></div>
                                <div class="card-box table-responsive">
                                    <h4 class="m-t-0 header-title"><b>Room Type Tables</b></h4>
                                    <p class="text-muted font-14 m-b-30">
                                    </p>
                                    <div class="row">
                                        <div class="col-sm-6">
                                            <div class="m-b-30">
                                                <a href="<?php echo base_url(); ?>adm/Room/add_room_type" id="addToTable" class="btn btn-success waves-effect waves-light">Add <i class="mdi mdi-plus-circle-outline"></i></a>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <table id="datatable" class="table table-striped table-bordered">
                                        ...
                                        <tr>
                                            ...
                                            <th>Room Type Create Date</th>
                                            <th>Action</th>
                                        </tr>
                                        </thead>


                                        <tbody id="show_data_room_type">
                                        </tbody>
                                    </table>
                                </div>
                            </div>
        ...
        <div id="Delete" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="javascript:delete_room_type();" method="post" id="form-delete">
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                view_table_room_type();
                $('#datatable').dataTable();
            });
          
            function view_table_room_type(){
                $.ajax({
                    type  : 'ajax',
                    url   : '<?php echo base_url(); ?>adm/Room/view_room_type',
                    async : false,
                    dataType : 'json',
                    success : function(data){
                        ...
                        for(i=0; i<data.length; i++){
                            var datas = '\''+data[i].id_room_type+'\'';
                            html += '<tr>'+
                                    '<td>'+ no++ +'</td>'+
                                    '<td>'+data[i].room_type_name+'</td>'+
                                    '<td>'+data[i].room_type_createdate+'</td>'+
                                    '<td class="actions"><a href="<?php echo base_url(); ?>adm/Room/view_update_room_type/'+data[i].id_room_type+'"><button type="button" class="btn btn-icon waves-effect waves-light btn-primary"> <i class="fa fa-pencil"></i> </button></a> <button type="button" data-toggle="modal" data-target="#Delete" onclick="javascript:setValueidroomtype('+datas+');" class="btn btn-icon waves-effect waves-light btn-danger"> <i class="fa fa-trash"></i> </button></td>'+
                                    '</tr>';
                        }
                        $('#show_data_room_type').html(html);
                    }
     
                });
            }
            function setValueidroomtype(id_room_type)
            {
                $("#room_type").val(id_room_type);
            }
            function delete_room_type()
            {
                $.ajax({
                    url:"<?php echo base_url() ?>adm/Room/delete_room_type",
                    type:"POST",
                    dataType:"JSON",
